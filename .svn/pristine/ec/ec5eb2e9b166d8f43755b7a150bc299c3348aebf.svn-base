...
*/
class index_controller extends mobile_controller{
	function waptpl($tpname){
		$this->yuntpl(array('mobile/member/user/'.$tpname));
	}
	...
	function sq_action(){
		...
		$pageurl=Url('mobile',$urlarr,'member');
	...
	function password_action(){
		if($_POST['submit']){
			...
			if($pw!=$member['password']){
				...
				$data['msg']="修改成功，请重新登录！";
				$data['url']=$this->config['sy_weburl'].'/mobile/index.php?m=login';
			}
