...
class map_controller extends common{
    ...
    function joblist_action(){
        ...
        foreach($CompanyList as $k1=>$v1){
            ...
            $json_list[$k1]['com_url']=Url('mobile',array('c'=>'company','a'=>'show','id'=>$v1['uid']));
            foreach($CompanyList[$k1]['joblist'] as $k2=>$v2){
                $json_list[$k1]['joblist'][]=array('name'=>yun_iconv('gbk','utf-8',$v2['name']),'job_url'=>Url('mobile',array('c'=>'job','a'=>'view','id'=>$v2['id'])));
            }
        }
